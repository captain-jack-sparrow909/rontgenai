...
export function ForgeAtmosphere() {
  return (
    <div className="pointer-events-none absolute inset-0 -z-10 overflow-hidden" aria-hidden>
      <div className="absolute -left-24 top-0 h-[400px] w-[400px] rounded-full bg-rose-500/15 blur-[120px]" />
      <div className="absolute -right-16 top-36 h-[320px] w-[320px] rounded-full bg-pink-600/15 blur-[100px]" />
      <div className="absolute bottom-0 left-1/3 h-[240px] w-[400px] rounded-full bg-fuchsia-600/10 blur-[110px]" />
      <div
        className="absolute inset-0 opacity-[0.12]"
        style={{
          backgroundImage:
            "linear-gradient(rgba(251,113,133,0.08) 1px, transparent 1px), linear-gradient(90deg, rgba(251,113,133,0.08) 1px, transparent 1px)",
  ...
  return (
    <div
      className={cn(
        ...
        glow && "shadow-rose-500/10 ring-1 ring-rose-400/20",
        className,
      )}
    >
...
export function ForgeLabel({
  ...
  return (
    <div className="mb-3 flex items-center gap-2">
      {index ? (
        <span className="font-mono text-[10px] tracking-widest text-rose-400/70">
          {index}
        </span>
      ) : null}
